...
export default async function SupplierProfilePage({ params }: PageProps) {
  ...
  const getAlertBorderColor = (severity: RiskLevel) => {
    switch (severity) {
      ...
      case 'high':
        return 'border-l-destructive'
      case 'medium':
        return 'border-l-tertiary-fixed-dim'
      default:
        return 'border-l-secondary'
    }
  }

  const getAlertBadgeClasses = (severity: RiskLevel) => {
    switch (severity) {
      case 'critical':
  ...
  const getAlertLabel = (severity: RiskLevel) => {
    switch (severity) {
      case 'critical':
        return 'Crítico'
      case 'high':
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-20 items-start">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            {supplier.alerts.map((alert) => (
              <div
                key={alert.id}
                className={`bg-surface-container-lowest border border-outline-variant p-6 border-l-4 ${getAlertBorderColor(
                  alert.severity
                )}`}
              >
                <div className="flex justify-between items-start mb-2">
                  <span
                    className={`px-2 py-0.5 text-[11px] font-semibold rounded uppercase ${getAlertBadgeClasses(
                      alert.severity
                    )}`}
                  >
